...
from critic_AC_3 import Critic
...
choose_target = ['target_electronics']
save4dir=os.path.join(savevar_path,choose_target[0])
if not os.path.exists(save4dir):
	os.makedirs(save4dir)
for item_target in choose_target:
    ...
    for i in range(1):
        ...
        critic.pre_train(X_train, y_train, X_test, y_test, 2, batch_size_critic, model_path=model_path)
        ...
        '''
        sess = tf.Session()
        actor =SDG(sess, n_steps=bag_size, input_size=data_features, output_size=1, cell_size=hidden_size,batch_size=batch_size_actor, lr=lr_SDG, repr=W_data)
        initializer = tf.global_variables_initializer()
        sess.run(initializer)
        '''
        ...
        for actor_epoch in range(pre_epochs_actor):
            ...
            td_sum_save={}
            td_sum=0
            for bag_id in range(num_train // bag_size):              #every bag is a step in MDP
        #        createVar['g_' + str(bag_id)] =tf.Graph()
...
#            for actor_epoch in range(pre_epochs_actor):
                ...
                if bag_id == 0:
                    ...
                    select_label = y_train_t    ##new!!!!9-19
        ...
        #        select_train, select_label,select_num1 = actor.sample(tf.convert_to_tensor(state), X_train_t, y_train_t)
                state_ = actor.deform(W_data_t_3d)
                state_=np.squeeze(state_)
                print("squ", state_)
                select_train_, select_label_,select_num2 = actor.sample(tf.convert_to_tensor(state_), X_train_t, y_train_t)
                # print(select_train_)
                # print(select_label_)
                # print("*****state:", state)
                ...
                #select_num1_save["select_num1"+"_bag_id_"+str(bag_id)]=select_num1
                select_num2_save["select_num2"+"_bag_id_"+str(bag_id)]=select_num2			

                    #td_loss=0.05
        #        with globals()['g_' + str(bag_id)].as_default():
        #        actor.learn(W_data_t_3d, td_loss)
                state = state_
                select_train = select_train_   ##new!!!!9-19, update current selected and former selected alternatively
                select_label = select_label_
               
                loss4test, acct = critic.test(X_test, y_test, batch_size_critic)
                acc_save["acct"+"_bag_id_"+str(bag_id)]=acct
                loss4test_save["loss4test"+"_bag_id_"+str(bag_id)]=loss4test
                if best_acc < acct:
                   best_acc = acct
                    # saver.save(sess, best_path_actor)
                best_save["best_acc"+"_bag_id_"+str(bag_id)]=best_acc
       ...
       #     critic.model_update()
            save_acc_loss_dict["acc_save"]=acc_save 
            save_acc_loss_dict["tderror_save"]=tderror_save
            save_acc_loss_dict["best_save"]=best_save
            save_acc_loss_dict["w_data_t_save"]=W_data_t_save
            #save_acc_loss_dict["state_t_save"]=state_t_save
            save_acc_loss_dict["loss4test_save"]=loss4test_save
            save_acc_loss_dict["x_data_t_dev_save"]=X_data_t_dev_save
            save_acc_loss_dict["select_num1_save"]=select_num1_save
            save_acc_loss_dict["select_num2_save"]=select_num2_save
            pickle.dump(save_acc_loss_dict,save_acc_loss_dict_path)
        ...
        for ep in range(epochs):
            ...
            for bag_id in range(num_train // bag_size):  # partition the dataset uniformly into bags of size: bag_size
                ...
                W_data_t = critic.get_repr(X_train_t)
                W_data_t_3d = np.reshape(W_data_t, [-1, bag_size, W_data_t.shape[1]])
                select_train, select_label = actor.sample(tf.convert_to_tensor(state), X_train_t, y_train_t)
                # print(np.reshape(W_data, [-1, bag_size, W_data.shape[1]]).size)
                state_ = actor.deform(W_data_t_3d)
                select_train_, select_label_ = actor.sample(tf.convert_to_tensor(state_), X_train_t, y_train_t)
                td_error, acc, acc_= critic.predict_self(select_train, select_label, select_train_, select_label_, X_dev, y_dev,
                                        epochs=1,
                                        batch_size=1)
                actor.learn(W_data_t_3d, td_error)
                state = state_
