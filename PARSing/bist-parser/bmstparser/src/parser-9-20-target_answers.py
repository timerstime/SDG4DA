...
print("item_targetitem_targetitem_targetitem_targetitem_targetitem_targetitem_targetitem_targetitem_target")
...
choose_target=['target_answers']
print("choose_target",choose_target)
        ...
        for actor_epoch in range(pre_epochs_actor):
            ...
            for bag_id in range(num_train // bag_size):
                ...
                if bag_id == 0:
                    ...
                    select_label = []
				'''
                #X_data_t_dev=parser.get_repr_self(X_dev)
                ...
                select_train, select_label = actor.sample(tf.convert_to_tensor(state),X_train_t)
                state_ = actor.deform(W_data_t_3d)
                state_ = np.squeeze(state_)
                print("squ", state_)
                select_train_, select_label_ = actor.sample(tf.convert_to_tensor(state_),
                                                                                         X_train_t)
