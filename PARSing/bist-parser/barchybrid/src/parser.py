...
if __name__ == '__main__':
    parser = OptionParser()
    parser.add_option("--train", dest="conll_train", help="Annotated CONLL train file", metavar="FILE", default="../../save_parsing/emails_newsgroups_answers_reviews_weblogs_target_wsj/train_dev.conll")
    parser.add_option("--dev", dest="conll_dev", help="Annotated CONLL dev file", metavar="FILE", default="../../save_parsing/emails_newsgroups_answers_reviews_weblogs_target_wsj/test.conll")
    parser.add_option("--test", dest="conll_test", help="Annotated CONLL test file", metavar="FILE", default="")
    parser.add_option("--params", dest="params", help="Parameters file", metavar="FILE", default="params.pickle")
    parser.add_option("--extrn", dest="external_embedding", help="External embeddings", metavar="FILE",default="../../model/sskip.100.vectors")
    ...
    parser.add_option("--dynet-mem", type="int", dest="cnn_mem", default=1500)
    ...
    if not options.predictFlag:
        ...
        if not conllu:
            os.system('perl src/utils/eval.pl -g ' + options.conll_test + ' -s ' + tespath  + ' > ' + 'temp' + '.txt')
        else:
