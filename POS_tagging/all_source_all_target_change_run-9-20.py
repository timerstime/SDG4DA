...
class NNTagger(object):
    ...
    def get_avg_nonlayer(self,data1,data2):
        ...
        distance = similarity.renyi_divergence(non_layer_avg_data1,non_layer_avg_data2)
    ...
    def predict_self(self, X_test, y_test, X_test_, y_test_, X_dev,word_dropout_rate=None,model_path=None,minibatch_size=None,log_losses=None):
        ...
        return self.td_loss,total_loss2,total_loss2
...
for item_target in choose_target:
    ...
    if args.patience:
        if not args.dev or not args.save:
            print("patience requires a dev set and model path (--dev and --save)")
            exit()
    args.save=args.save+choose_target[0]
    if args.save:
        # check if folder exists
        if os.path.isdir(args.save):
            if not os.path.exists(args.save):
    ...
    sess = tf.Session()
    actor = SDG(sess, n_steps=bag_size, input_size=data_features, output_size=1,
    ...
    for actor_epoch in range(pre_epochs_actor):
        ...
        for bag_id in range(num_train // bag_size):
            ...
            X_data_t_dev=tagger.get_repr(X_dev)
            ...
            #select_train, select_label,select_count= globals()['actor__' + str(bag_id)].sample(tf.convert_to_tensor(state),
            #                                                                       X_train_t, y_train_t)
            state_ = actor.deform(W_data_t_3d)
            state_ = np.squeeze(state_)
            state_t_save["state_"+"_bag_id_"+str(bag_id)]=state_
            print("squ", state_)
            select_train_, select_label_,select_count_= actor.sample(tf.convert_to_tensor(state_),
                                                                                     X_train_t, y_train_t)
            # print(select_train_)
            # print(select_label_)
            # print("*****state:", state)
            select_count_save["select_count_save_"+"_bag_id_"+str(bag_id)]=select_count_
...
#                globals()['actor__' + str(bag_id)].learn(W_data_t_3d, td_loss)
            ...
            if best_acc < acct:
                best_acc = acct
                save(tagger, args.save+"actor_epoch_"+str(actor_epoch)+"_bag_id_"+str(bag_id))
        ...
        save_acc_loss_dict["select_count_save"]=select_count_save
